...
describe("POST /companies", function () {
  ...
  test("fail for users with no admin auth", async function () {
    const resp = await request(app)
      .post("/companies")
      .send(newCompany)
...
describe("GET /companies", function () {
  ...
  // the route will check for variations of query filtering params

  test("should return filtered companies", async () => {
    const resp = await request(app).get("/companies?minEmployees=3");
    console.log(resp.body);
    expect(resp.statusCode).toBe(200);
    expect(resp.body.companies[0]).toHaveProperty("handle", "c3");
    expect(resp.body.companies[0]).toHaveProperty("description", "Desc3");
  });

  test("should throw an error for incorrect employess type(must be int)", async () => {
...
describe("PATCH /companies/:handle", function () {
  test("works for users wth admin", async function () {
    const resp = await request(app)
      .patch(`/companies/c1`)
      .send({
        name: "C1-new",
  ...
  test("should not work, no admin", async function () {
    const resp = await request(app)
      .patch(`/companies/c1`)
      .send({
        name: "C1-new",
...
describe("DELETE /companies/:handle", function () {
  ...
  test("fails for users with no admin", async function () {
    const resp = await request(app)
      .delete(`/companies/c1`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body.error.message).toEqual("must be admin");
